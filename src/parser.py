...
def p_modificador_tipo_lista(p): # AST_tipo_lista
  '''
  modificador_tipo : ABRE_CORCHETE CIERRA_CORCHETE
  '''
  s = AST_sintaxis(p[1])
...
def p_modificador_tipo_acceso(p): # AST_modificador_objeto_acceso
  '''
  modificador_tipo : ACCESO s IDENTIFICADOR opt_modificador_tipo
  '''
  s = AST_sintaxis(p[1])                  # AST_sintaxis
  s = concatenar(s, p[2])                 # [AST_skippeable]
...
def p_tipo_parentesis(p): # AST_tipo (puede ser AST_tipo_funcion o un AST_tipo cualquiera entre paréntesis)
  '''
  tipo_sin_id : ABRE_PAREN s tipo_o_parametros opt_modificador_tipo
  '''
  s = AST_sintaxis(p[1])    # AST_sintaxis
  s = concatenar(s, p[2])   # AST_skippeable
  rec = p[3]                # AST_tipo (puede ser AST_tipo_funcion o un AST_tipo cualquiera entre paréntesis)
  opt_adicional = p[4]      # AST_tipo_lista | AST_tipo_suma | AST_modificador_objeto_acceso | [AST_skippeable]
  rec.apertura(s)
  rec.modificador_adicional(opt_adicional)
  p[0] = rec
...
def p_tipo_o_parametros_flecha(p): # AST_tipo_funcion
  '''
  ...
  tipo_salida = p[6]                # AST_identificador | AST_tipo_objeto | AST_tipo_funcion
...
def p_campo_tipo(p): # AST_campo_tipo
  ...
  s = p[2]                  # [AST_skippeable]
  dp = AST_sintaxis(p[3])   # AST_sintaxis
  s = concatenar(s, dp)     # [AST_skippeable]
  s = concatenar(s, p[4])   # [AST_skippeable]
  tipo_campo = p[5]         # AST_tipo
...
def p_modificador_objeto_acceso(p): # AST_modificador_objeto_acceso
  '''
  modificador_objeto_expresion_no_vacio : ACCESO s IDENTIFICADOR opt_modificador_expresion_o_invocacion
  '''
...
def p_modificador_objeto_index(p): # AST_modificador_objeto_index
  '''
  modificador_objeto_expresion_no_vacio : ABRE_CORCHETE s expresion CIERRA_CORCHETE opt_modificador_expresion_o_invocacion
  '''
  abre = AST_sintaxis(p[1])
  s = concatenar(abre, p[2])  # [AST_skippeable]
...
def p_opt_modificador_expresion_o_invocacion_invocacion(p):
  '''
  opt_modificador_expresion_o_invocacion : invocacion
  '''
  p[0] = p[1]

def p_opt_modificador_expresion_o_invocacion_modificador(p):
  '''
  opt_modificador_expresion_o_invocacion : opt_modificador_expresion
  '''
  p[0] = p[1]
...
def p_expresion_asignada_new(p): # AST_invocacion
  '''
  expresion_asignada_sin_pre : NEW s expresion_sin_invocacion s invocacion
  '''
  ...
  clase.apertura(s1)
  clase.clausura(s2)
  p[0] = aplicarModificador(clase, argumentos)
...
def p_expresion_new(p): # AST_invocacion
  '''
  expresion_sin_pre_ni_id : NEW s expresion_sin_invocacion s invocacion
  '''
  new = AST_sintaxis(p[1])                  # AST_sintaxis
  s1 = concatenar(new, p[2])                # [AST_skippeable]
  clase = p[3]                              # AST_expresion
...
def p_expresion_entre_parentesis(p): # AST_expresion
  ...
  if isinstance(expresion, AST_nodo):
    ...
    if isinstance(modificador_expresion, AST_nodo):
...
def p_declaracion_literal(p): # AST_expresion_literal
  ...
  '''
  literal = AST_expresion_literal(p[1])   # AST_expresion_literal
  modificador = p[2]                      # AST_modificador_operador | [AST_skippeable]
  p[0] = aplicarModificador(literal, modificador)

def p_opt_modificador_literal_suelto_con_skip(p): # AST_modificador_operador | [AST_skippeable]
  '''
...
def p_opt_modificador_literal_suelto_sin_skip(p): # AST_modificador_operador | [AST_skippeable]
  '''
  opt_modificador_literal_suelto : modificador_literal_suelto
  '''
  modificador_literal_suelto = p[1]       # AST_modificador_operador | [AST_skippeable]
...
def p_modificador_literal_suelto_no_vacio(p): # AST_modificador_operador
  '''
...
def p_modificador_literal_suelto_operador(p): # AST_modificador_operador
  '''
...
def p_modificador_objeto_expresion_asignacion(p): # AST_expresion
  '''
...
def p_clave_campo_variable(p): # AST_identificador
  ...
  opt_decorador = p[4]                      # AST_decorador_tipo | [AST_skippeable]
...
def p_declaracion_dentro_de_clase_con_pre(p):
  '''
  declaracion_dentro_de_clase : identificador_con_modificadores_pre opt_modificador_dentro_de_clase
  '''
  declaracion = p[1]                  # AST_identificador
...
def p_identificador_con_modificadores_pre(p): # AST_identificador | AST_identificadores
  '''
  identificador_con_modificadores_pre : identificador_uno opt_mas_identificadores_o_modificadores
  '''
  identificador = p[1]                      # AST_identificador
  rec = p[2]                                # AST_identificador | AST_identificadores | [AST_skippeable]
...
def p_opt_mas_identificadores_o_modificadores_con_skip(p): # AST_identificador | AST_identificadores | [AST_skippeable]
  '''
  opt_mas_identificadores_o_modificadores : sf mas_identificadores_o_modificadores
  '''
  s = p[1]                  # [AST_skippeable]
  identificador = p[2]      # AST_identificador | AST_identificadores | [AST_skippeable]
  p[0] = modificador_con_skip(identificador, s)

def p_opt_mas_identificadores_o_modificadores_sin_skip(p): # AST_identificador | AST_identificadores | [AST_skippeable]
  '''
  opt_mas_identificadores_o_modificadores : mas_identificadores_o_modificadores
  '''
  p[0] = p[1]

def p_mas_identificadores_o_modificadores_vacio(p): # [AST_skippeable]
  '''
  mas_identificadores_o_modificadores : vacio
  '''
  p[0] = p[1]

def p_mas_identificadores_o_modificadores_no_vacio(p): # AST_identificador | AST_identificadores
  '''
  mas_identificadores_o_modificadores : mas_identificadores_o_modificadores_no_vacio
  '''
  p[0] = p[1]
...
def p_identificadores_con_modificadores_pre(p): # AST_identificadores
  '''
  mas_identificadores_o_modificadores_no_vacio : ABRE_LLAVE s identificadores CIERRA_LLAVE
  '''
  abre = AST_sintaxis(p[1])   # AST_sintaxis
  s = concatenar(abre, p[2])  # [AST_skippeable]
  lista = p[3]                # [AST_identificador]
...
def p_modificador_id_clase_tupla(p): # AST_tipo_tupla
  '''
  modificador_id_clase_no_vacio : MENOR s tipos MAYOR
  '''
  abre = AST_sintaxis(p[1])       # AST_sintaxis
  abre = concatenar(abre, p[2])   # [AST_skippeable]
